...
from portfolio_analysis.scripts.constants.color import RED, ORANGE, BROWN, PINK, PURPLE
...
def _random_portfolios_scatter(
    mean_returns, cov_matrix, risk_free_rate, n_portfolios=2000
):
...
def _sample_efficient_frontier(mean_returns, cov_matrix, risk_free_rate, n_points=50):
    ...
    ret_minVol, _, _ = ef_min.portfolio_performance(
    ...
    ret_maxSharpe, _, _ = ef_max.portfolio_performance(
        risk_free_rate=risk_free_rate, verbose=False
    )
    ...
    for tr in test_returns:
        try:
            ...
            ret, vol, _ = ef_sample.portfolio_performance(
                risk_free_rate=risk_free_rate, verbose=False
            )
...
def optimize(
    ...
    df_random = _random_portfolios_scatter(
        mean_returns, cov_matrix, risk_free_rate, n_portfolios
    )
    ...
    chosen_ret, chosen_vol, chosen_sharpe = ef_strategy.portfolio_performance(
        risk_free_rate=risk_free_rate, verbose=False
    )
    port_opt[MAX_SHARPE] = {
        "ret": float(chosen_ret),
    ...
    frontier_vols, frontier_rets = _sample_efficient_frontier(
        mean_returns, cov_matrix, risk_free_rate, n_points=50
    )
    frontiers = {"vols": frontier_vols, "rets": frontier_rets}
...
def optimize_and_plot(
    ...
    for strategy, color in zip(
        port_opt.keys(), [f"{ORANGE}", f"{BROWN}", f"{PURPLE}", f"{PINK}"]
    ):
        fig.add_trace(
            go.Scatter(
    ...
    if current_portfolio_ret_vol is not None:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[cur_ret],
                mode="markers",
                marker=dict(color=f"{RED}", size=12, symbol="diamond"),
...
def render_optimization_tab(mean_returns, cov_matrix, current_portfolio_ret_vol=None):
    """
    A Streamlit UI that:
    ...
    if strategy == "Target Return":
        user_input_tr = st.number_input(
            "Desired Annual Return (%)", value=10.0, step=1.0
        )
...
def main():
    ...
    mean_returns = pd.Series(
        [0.15, 0.12, 0.2, 0.10], index=tickers
    )  # e.g. 15%, 12%, 20%, 10%
